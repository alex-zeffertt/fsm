  ...
  public:
    ...
    void inject_event(Event event, void *context = nullptr)
    {
        ...
        while (event != Derived::EV_NULL_EVENT)
        {
            ...
            reinterpret_cast<Derived*>(this)->log_event(event);
            ...
            {
                // Change state
                event = reinterpret_cast<Derived*>(this)->handle_action(action, context);

                if (_state != next_state)
                {
                    // How this is handled is up to the derived class
                    reinterpret_cast<Derived*>(this)->log_state(_state, next_state);
